...
dmaDestBuff_32bit = sin(2*pi*2.2e3*tt);
...
k = 8256/2;
j0 = 0+1; % allpass starts with odd inputs and skips by 2
j1 = 0+1; % output pointer into array
while(k > 0)  % each loop produces 2 outputs at 2X rate, so use DMALEN/8

	in0 = dmaDestBuff_32bit(j0);
    j0 = j0 + 1;
    
    in1 = 	dmaDestBuff_32bit(j0);
    j0=j0+1;
    ...
    ap_state_stage2_zm0 = (in1 /2) - denom0(2)*ap_state_stage2_zm1 - denom0(3)*ap_state_stage2_zm2;
    ...
    ap_state_stage2_del_zm0 = (in0 /2) - denom1(2)*ap_state_stage2_del_zm1;
